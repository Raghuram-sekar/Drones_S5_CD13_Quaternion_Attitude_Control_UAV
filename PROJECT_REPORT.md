...
Conventional Euler angle controllers ($\phi, \theta, \psi $) suffer from gimbal lock singularities at extreme pitch angles ($\theta = \pm 90°$) and severe cross-coupling between elevator and rudder surfaces at steep bank angles ($80° - 90°$). By executing outer-loop attitude control entirely within unit quaternion space ($ SO(3)$), the proposed controller achieves singularity-free attitude tracking, shortest-path rotation arcs via SLERP, and natural aerodynamic decoupling. 

In 6-DOF dynamic flight simulations modeling an Extra 330SC aircraft, the quaternion controller maintained pitch tracking error below $0.27°$ across all bank angles up to $90°$ knife-edge flight, whereas the classical Euler controller exhibited severe failure with yaw tracking error exploding to $30.50°$.
...
To visualize how the aircraft acts in real-time under Quaternion vs. Euler attitude control, an interactive desktop simulator is provided:
...
### 4.4 Atmospheric Wind Turbulence & Crosswind Gust Rejection Benchmark

To evaluate flight stability under realistic atmospheric disturbances, a **Dryden Wind Turbulence Model** (MIL-F-8785C / MIL-STD-1797A) integrated with a **$15\text{ m/s}$ crosswind step gust** ($v_{gust} = 15\text{ m/s}$ applied at $t = 8.0\text{s}$) was executed during an aggressive $80^\circ$ bank angle maneuver.

#### Quantitative Gust Rejection Benchmark Table:

| Metric / Parameter | Quaternion ($Q_P$) Controller | Classical Euler PID Controller | Performance Summary |
| :--- | :---: | :---: | :--- |
| **Pitch Error Peak during Gust** | **1.37°** | 11.08° | **8x reduction in transient pitch deviation** |
| **Yaw Error Peak during Gust** | **4.61°** | 7.33° | Significant suppression of lateral sideslip |
| **Overall Yaw Tracking RMSE** | **3.05°** | 3.56° | Superior trajectory holding under severe gust |
| **Gust Settlement Time** | **< 0.45 s** | > 3.20 s (Unstable) | **Quaternion recovers 7x faster with zero residual oscillation** |

#### Benchmark Figure:
![Atmospheric Turbulence and Gust Rejection Benchmark](figures/fig_turbulence_rejection.png)
*Figure 4.7: 6-DOF Aircraft Simulation Response under Dryden Atmospheric Turbulence & 15 m/s Crosswind Gust at t = 8.0s.*
